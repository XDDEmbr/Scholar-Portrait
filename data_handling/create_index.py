...
from utils.load_dir import data_dir
...
class FaissIdx:
    ...
    def load_index(self, index_path):
        ...
        # Load Index
        self.index = faiss.read_index(join(index_path, "index_test.faiss"))


        # Load Ids
        with open(join(index_path, "ids_test.pkl"), "rb") as f:
...
# 加载arxiv 数据集的预处理版本，初始化Faiss 索引；如果索引文件已经存在，则会从文件中加载索引和文档 ID；
# 接下来，将数据集中的文本与 ID 转换为 Pandas DataFrame 的形式，并调用 add_doc 方法将文本向量添加到 Faiss 索引中；
# 最后，将 Faiss 索引和文档 ID 存储到硬盘上；
if __name__ == "__main__":
    # Load data
    print("[+] Loading data")
    if not os.path.exists(join(data_dir, "arxiv_processed_test.csv")):
        print(f"[!] File {join(data_dir, 'arxiv_processed_test.csv')} not found. Please download the data first")
        exit()
